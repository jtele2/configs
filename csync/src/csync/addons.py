...
class AddonManager:
    ...
    def setup_addons(self):
        ...
        console.print(
            "[bold magenta]🎨 Setting up addons and plugins...[/bold magenta]"
        )
        ...
        if setup_script.exists():
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    console.print(
                        f"[yellow]⚠️  Plugin installation had issues: {result.stderr}[/yellow]"
                    )
        ...
        if not omz_path.exists():
            if Confirm.ask(
                "[yellow]💎 Oh My Zsh not found. Would you like to install it?[/yellow]"
            ):
        ...
        if os.environ.get("ZSH_VERSION") and (Path.home() / ".zshrc").exists():
            ...
            console.print(
                "[yellow]💡 Restart your shell or run 'source ~/.zshrc' to load changes[/yellow]"
            )
