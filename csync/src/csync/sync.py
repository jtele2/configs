...
class Syncer:
    ...
    def sync(self, force_push=False, force_pull=False, dry_run=False, background=False):
        ...
        if force_push:
            ...
            if not dry_run:
                self.repo.remotes.origin.push(refspec=f"{self.config.branch}:{self.config.branch}", force=True)
        ...
        if local_commit != remote_commit:
            ...
            if dry_run:
                ...
                try:
                    ...
                    if not background:
                        console.print("[yellow]⚠️  Rebase failed, attempting merge...[/yellow]")
                    self.repo.git.rebase("--abort")
                    
                    try:
                        origin.pull(rebase=False)
                    except git.GitCommandError:
                        console.print("[red]❌ Merge failed. Manual intervention required.[/red]")
                        console.print("[yellow]💡 Try: --force-pull or --force-push[/yellow]")
                        self.config.update_sync_status("✗")
                        return False
        else:
            if not background:
        ...
        if self.repo.is_dirty():
            ...
            if dry_run:
                ...
                self.repo.index.add("*")
                commit_msg = f"Sync from {machine_id} at {datetime.now():%Y-%m-%d %H:%M:%S}"
        ...
        if not dry_run:
            self.config.last_sync_file.write_text(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def sync_marked_files(self):
        ...
        for rel_path in marked_files:
            ...
            if not file_path.is_symlink() or file_path.resolve() != external_path.resolve():
                if file_path.exists() or file_path.is_symlink():
                    file_path.unlink()
                
                file_path.symlink_to(external_path)
                console.print(f"  [green]✓ Linked: {file_path} → {external_path}[/green]")
                synced_count += 1
